...
//    }
    ...
    const AVCodec *const_video_codec = video_codec;
    video_stream_index = av_find_best_stream(avformat_context, AVMEDIA_TYPE_VIDEO, -1, -1, &const_video_codec, 0);
    if (video_stream_index < 0) {
        printf("av_find_best_stream faliture\n");
        return;
    }
    
    const AVCodec *const_audio_codec = audio_codec;
    audio_stream_index = av_find_best_stream(avformat_context, AVMEDIA_TYPE_AUDIO, -1, -1, &const_audio_codec, 0);
