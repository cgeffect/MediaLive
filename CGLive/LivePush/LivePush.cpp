...
/*read 1 byte*/
inline int ReadU8(uint32_t *u8,FILE*fp){
...
/*read 2 byte*/
inline int ReadU16(uint32_t *u16,FILE*fp){
...
/*read 3 byte*/
inline int ReadU24(uint32_t *u24,FILE*fp){
...
/*read 4 byte*/
inline int ReadU32(uint32_t *u32,FILE*fp){
...
inline int PeekU8(uint32_t *u8,FILE*fp){
...
inline int ReadTime(uint32_t *utime,FILE*fp){
...
int LivePush::pushUsePacket(const char* argv){
    RTMP *rtmp=NULL;
    ...
    fseek(fp,9,SEEK_SET); //定位到文件开头
    //jump over previousTagSizen
    fseek(fp,4,SEEK_CUR);//从当前位置开始
    ...
    while(1)
    {
        ...
        printf("type: %d, len: %d, time: %d, id: %d\n", type, datalength, timestamp, streamid);
...
int LivePush::pushUseWrite(const char *file){
    uint32_t start_time=0;
    uint32_t now_time=0;
