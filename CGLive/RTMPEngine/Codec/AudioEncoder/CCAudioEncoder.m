...
//编码器回调函数, 回调函数中主要做的是为即将编码的数据进行赋值,即我们要把原始音频数据赋值给回调函数中的ioData参数,这是我们在编码前最后一次控制原始音频数据,此回调函数执行后即完成了编码的过程
...
- (void)encodeAudioSamepleBuffer: (CMSampleBufferRef)sampleBuffer {
    ...
    dispatch_async(_encoderQueue, ^{
        ...
        //4.获取BlockBuffer中音频数据大小以及音频数据地址, _pcmBuffer指向音频数据的地址
        ...
        if (status == noErr) {
            ...
            //释放pcmBuffer
            free(pcmBuffer);
            if ([CCAudioConfig defaultConifg].isNeedADTS) {
                ...
                dispatch_async(self->_callbackQueue, ^{
                    [self->_delegate audioEncodeCallback:fullData];
